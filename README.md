...
- **Fast WiFi connect:** to cut the radio-on time of a wakeup, the last-good AP (BSSID + channel) is cached in RTC RAM for a scan-free reconnect (on by default, `iot.setFastReconnect()`), and an optional static IP (`iot.setStaticIp()`) skips DHCP. Both fall back automatically to a full-scan/DHCP connect. The connect duration is exposed as `wifi_connect_ms` telemetry.
...
- **System-telemetry firmware fields changed (3.1).** The old `firmware_version` field (the composite ESP application descriptor, which in a PlatformIO/Arduino build identifies the framework rather than the sketch) is now sent as **`firmware_id`**. `firmware_version` now carries the clean version and a new `firmware_commit` the git commit, both derived from the consuming project's git at build time (a pre-build script shipped via `library.json`); override with `iot.setFirmwareVersion()`/`setFirmwareCommit()` or disable git with `-DIOT_NO_GIT_VERSION`. The `Iot::getFirmwareVersion()` method likewise now returns the clean version; the composite is `Iot::getFirmwareId()`. Update dashboards/queries that referenced `firmware_version`.
